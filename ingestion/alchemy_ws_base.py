...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [BASE-PRODUCER] %(levelname)s %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
log = logging.getLogger(__name__)

# ── Only these three lines differ from the ETH producer ──────────────────────
ALCHEMY_KEY   = os.getenv("ALCHEMY_BASE_KEY")
WS_URL        = f"wss://base-mainnet.g.alchemy.com/v2/{ALCHEMY_KEY}"
HTTP_URL      = f"https://base-mainnet.g.alchemy.com/v2/{ALCHEMY_KEY}"
KAFKA_SERVERS = os.getenv("KAFKA_BOOTSTRAP_SERVERS", "localhost:9092")
KAFKA_TOPIC   = os.getenv("BASE_TOPIC", "base_raw_transactions")
CHAIN         = "base"
# ─────────────────────────────────────────────────────────────────────────────
...
def normalise_tx(tx: dict, block_timestamp_hex: str) -> dict:
    ...
    tx_fee_eth     = (gas_price * 21_000) / 1e18
...
def send_block_to_kafka(producer: KafkaProducer, block: dict) -> int:
    ...
    for tx in transactions:
        ...
        try:
            producer.send(KAFKA_TOPIC, key=normalised["tx_hash"], value=normalised)
            sent += 1
...
def main():
    if not ALCHEMY_KEY:
        log.error("ALCHEMY_BASE_KEY not set in .env — aborting.")
        sys.exit(1)
    ...
    try:
        ...
        log.info("Shutting down Base producer.")
    finally:
        producer.close()
